...
def calculate_F1(function):
   ...
   for summary_path, summary in human_summaries.items():
      ...
      if summary['title'] == "Around the World in Eighty Days":
         related_summaries = list(filter(
               lambda curr_summary: curr_summary['title'] == 'Around the World in 80 Days', human_summaries.values()))
      elif summary['title'] == "Around the World in 80 Days":
         related_summaries = list(filter(
               lambda curr_summary: curr_summary['title'] == 'Around the World in Eighty Days', human_summaries.values()))
      else:
         related_summaries = list(filter(lambda curr_summary: curr_summary['title'] == summary[
                                    'title'] and curr_summary['source'] != summary['source'], human_summaries.values()))
      ...
      data.append([mean_max_score, len(unique_sents), summary['title'], summary['source']])
...
def write_to_csv(function, split,filename):
   print(filename)
   df = pd.DataFrame(data, columns=[function, "number of Unique sentences", "title", "source"])
   # Save file.
   df.to_csv(f"../csv_results/booksum_summaries/book-comparison-results-{split}-{filename}.csv")
