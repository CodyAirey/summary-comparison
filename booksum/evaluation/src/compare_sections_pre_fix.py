...
def calculate_F1(function):
   ...
   for summary_path, summary in human_summaries.items():
      ...
      # number_of_matches[summary['source']][summary['chapter_title']]['total_sections'] += 1
      ...
      for path, summary2 in human_summaries.items():
         ...
         if summary["chapter_title"] == summary2["chapter_title"]:
            ...
            # break
      # if will_be_used:
      # number_of_matches[summary['source']][summary['book']]['total_sections_used'] += will_be_used

         # if summary['book'] == summary2['book']:


      # aggregate_flag = False
      # numeric_count = 0
      # split = summary['chapter_title'].split("-").reverse()
      # for token in split:
      #    if token.isnumeric(): numeric_count+=1
      #    if token in ("chapter", "chapters", "scene", "scenes", "part", "act", "book", "canto", "volume"): continue
      #    if token.isalpha(): break
      # aggregate_flag = numeric_count >= 2
